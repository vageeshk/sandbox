var gcsTheSchool = angular.module('gcsTheSchool', ['ui.router']);
(function () {
	config.$inject = ['$stateProvider'];
	function config($stateProvider) {
		$stateProvider
			.state('school', { url: "/school", params: { scrollTo: '' }, templateUrl: "app/the-school/theSchool.html", controller: 'schoolCtrl' })
			.state('school.teachingFraternity', { url: "/teaching-fraternity", templateUrl: "app/the-school/theSchool.html" })
			.state('school.schoolSchedule', { url: "/school-schedule", templateUrl: "app/the-school/schoolSchedule.html" })
			.state('school.trainTheTeacher', { url: "/train-the-teacher", templateUrl: "app/the-school/trainTheTeacher.html" })
			.state('school.achievements', { url: "/achievements", templateUrl: "app/the-school/achievements.html" })
			.state('school.news', { url: "/news", templateUrl: "app/the-school/news.html" });
	}
	gcsTheSchool.config(config);
} ());
...
(function () {
		...
		this.getNews = function () {
			var news = [
				...
				{ 
					...
					Thumbnail: 'http://res.cloudinary.com/hs5fb0l8g/image/upload/v1497002780/env-hp-10_urr8kv.jpg',
	...
	gcsTheSchool.service('schoolService', service);
} ());

//ctrl =>school controller
(function () {
	schoolCtrl.$inject = ['$scope', '$state', 'schoolService', 'appContext', '$log', '$stateParams', '$location', '$anchorScroll'];
	function schoolCtrl($scope, $state, schoolService, appContext, $log, $stateParams, $location, $anchorScroll) {
		if ($stateParams.scrollTo) {
			$location.hash($stateParams.scrollTo);
			$anchorScroll();
		}
		$scope.init = function () {
			var newsAvailable = appContext.news && appContext.news.length > 0;
			if (!newsAvailable) {
				appContext.news = schoolService.getNews();
				$scope.News = appContext.news;
			}
			else { $scope.News = appContext.news; }
		}
		$scope.init();
	}

	gcsTheSchool.controller('schoolCtrl', schoolCtrl);
} ());